...
class LoginScreen extends BaseRoute {
  final bool logoutsts;
  LoginScreen(this.logoutsts, {a, o}) : super(a: a, o: o, r: 'LoginScreen');
  @override
  _LoginScreenState createState() => new _LoginScreenState(this.logoutsts);
}
...
class _LoginScreenState extends BaseRouteState {
  ...
  _LoginScreenState(this.logoutsts) : super();
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: WillPopScope(
        onWillPop: () {
          logoutsts == true ? Navigator.of(context).push(
            MaterialPageRoute(
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      children: [
                        ...
                        //       child: Text(isLoginWithEmail ? 'Login' : '${AppLocalizations.of(context).txt_provide_number_desc}', style: Theme.of(context).primaryTextTheme.bodyText1),
                        //     ),
                        //   ],
                        // ),
                        // Visibility(
                        //   visible: isLoginWithEmail ? false : true,
                        //   child: Row(
                        //     mainAxisAlignment: MainAxisAlignment.start,
                        //     children: [
                        //       Padding(
                        //         padding: EdgeInsets.only(top: 2, bottom: 20, left: 12),
                        ...
                        Container(
                          ...
                          child: TextButton(
                              ...
                              child: Text(isLoginWithEmail ? '${AppLocalizations.of(context).btn_login}' : 'Login/ Signup')),
                        ),
          ...
          await apiHelper.login(_cPhone.text).then((result) async {
            if (result != null) {
              if (result.status == "1") {
                ...
                showSnackBar(key: _scaffoldKey, snackBarMessage: 'You are not yet registered with us, Complete quick signup process to explore the exiting offers and deals!');
                Navigator.of(context).push(
                  MaterialPageRoute(
  ...
  loginWithEmail() async {
    try {
      bool isConnected = await br.checkConnectivity();
      if (isConnected) {
        if (_cEmail.text != null && _cPassword != null && _cPassword.text.trim().isNotEmpty) {
          showOnlyLoaderDialog();
          await apiHelper.loginWithEmail(_cEmail.text.trim(), _cPassword.text.trim()).then((result) async {
            if (result != null) {
              if (result.status == "1") {
                ...
                bool isNumeric = _numeric.hasMatch(_cEmail.text);

                if(isNumeric) {
                  _currentUser.userPhone = _cEmail.text;
                } else {
          ...
          showSnackBar(key: _scaffoldKey, snackBarMessage: 'Please enter your mobile no / email!');
