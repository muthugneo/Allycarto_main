...
class AddBankDetails extends BaseRoute {
  ...
  final String namee;
  final String pann;
  AddBankDetails(this.address,  this.bankk, this.accc, this.ifscc, this.namee,this.pann, {a, o}) : super(a: a, o: o, r: 'AddAddressScreen');
  @override
  _AddAddressScreenState createState() => new _AddAddressScreenState(this.address, this.bankk, this.accc, this.ifscc, this.namee, this.pann);
}

class _AddAddressScreenState extends BaseRouteState {
  var _cPan = new TextEditingController();
  ...
  var address, bankk, accc, ifscc, namee, pann;
  _AddAddressScreenState(this.address, this.bankk, this.accc, this.ifscc, this.namee, this.pann) : super();
    ...
    try {
      if (_cName != null &&
        ...
        if (isConnected) {
          ...
          if (address == "0") {
            await apiHelper.addBank(_cName.text.toString(),_cBank.text.toString(),_cAcc.text.toString(),_cIfsc.text.toString(),_cPan.text.toString(),).then((result) async {
              if (result != null) {
                if (result.status == "200") {
                  hideLoader();
                  showSnackBar(key: _scaffoldKey, snackBarMessage: "Bank details updated successfully!");
            ...
            await apiHelper.addBank(_cName.text.toString(),_cBank.text.toString(),_cAcc.text.toString(),_cIfsc.text.toString(),_cPan.text.toString(),).then((result) async {
              if (result != null) {
                if (result.status == "200") {
                  ...
                  hideLoader();
                  showSnackBar(key: _scaffoldKey, snackBarMessage: "Bank details added successfully!");
                  Navigator.of(context).pop(true);
                } else if (result.status == "204") {
                  hideLoader();
                  showSnackBar(key: _scaffoldKey, snackBarMessage: "Pan Number Already Exists!");
                }
              }
            });
          }
      ...
      } else if (_cPan.text.isEmpty) {
        showSnackBar(key: _scaffoldKey, snackBarMessage: "Please enter pan number");
