...
class _CategoryListScreenState extends BaseRouteState {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("Local Shops"),),
      body: _shopList == null ? Center(child: CircularProgressIndicator()) : _shopList.length == 0 ? Center(child: Text("No data found!"))
        ...
        child: GridView.builder(
          ...
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
              childAspectRatio:(MediaQuery.of(context).size.width / 3) / 126.0,
              ...
              child: new Card(
                child: InkWell(
                  onTap: () {
                    ...
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (context) => VendorDetails(_shopList[i]["id"].toString(), _shopList[i]["name"], _shopList[i]["image"], _shopList[i]["address"],
                            _shopList[i]["latitude"].toString(), _shopList[i]["longitude"].toString(), _shopList[i]["description"] ?? "", a: widget.analytics, o: widget.observer),
                      ),
                    );
                  },
                  child: Container(
                                ...
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Text(
                                        '${_shopList[i]["status"]}',
                                        style: TextStyle(fontSize: 12,fontWeight: FontWeight.w600, color: _shopList[i]["status"] == "open" ? Colors.green : Colors.red),
                                        maxLines: 1,
                                      ),
                                    ],
                                  ),
